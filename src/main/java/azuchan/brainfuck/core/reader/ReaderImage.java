 ...
 * @version 1.4
 */
public class ReaderImage extends ReaderBF {
	...
	private void constructionWithPix(int rgb) throws IsNotBrainfuckInstructionException{
		for(Instructions i : Instructions.values()){
			if(rgb == i.getRGB()){
				constructionWithShortcut(Character.toString(i.getShortSyntax()));
				return ;
			}
		}
		...
		throw new IsNotBrainfuckInstructionException(Integer.toString(rgb));
	}
