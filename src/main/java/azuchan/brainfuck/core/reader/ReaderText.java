...
public class ReaderText extends ReaderBF{
	...
	protected void traitementLigne(String ligne) throws IsNotBrainfuckInstructionException{
		String l;
		if("#".equals(ligne)){
			l = " "; // pour éviter un split qui ne marcherait pas...
		}
		else{
			//         On vire le com   // On vire l'indentation
			l = ligne.split("#")[0].replaceAll("\t| ", "");
		}
		
		if(isLigneShortcut(l)){
			for(int i = 0; i < l.length(); i++){
				constructionWithShortcut(Character.toString(l.charAt(i)));
			}
		}
		else{
	...
	private boolean isLigneShortcut(String l){
		for(int i = 0; i < l.length(); i++){
			if(!(isShortcut(l.charAt(i)) || l.charAt(i) == ' ' || l.charAt(i) == '\t')){
				return false;
			}
		}
		return true;
